...
|메인페이지 스타일 및 기능 구현,  파이어스토어를 이용한 데이터베이스 연결 |전체디자인 구성 및 SCSS 공통화 작업(변수 선언, mixin 활용), 로그인, 회원가입 기능 구현| 사이드바 구현 및 뷰포트에 맞는 자동슬라이드,공지사항 구현 ,자료실 구현 |<div>휴가 리스트(드롭다운,상태관리,필터링) 구현</div> <div>휴가신청 페이지 구현</div> <div>최종배포</div> |이미지 파일 업로드 및 미리보기 구현, 유저 데이터(이메일, 이름, 전화번호) 불러오기 구현, 유저 이름 변경시 우측 상단 미니 배지에 수정된 이름 구현, 입력한 값으로 유저 데이터 수정 및 db데이터 수정, 입실 시간, 퇴실 시간 표시 구현, 휴가 신청 페이지 이동버튼 구현 |
